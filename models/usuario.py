...
# Para indicar que esta clase sera una tabla en la base de datos utilizaremos la conexion a la base de datos
class UsuarioModel(conexion.Model):
  ...
  # server_default solo puede aceptar string o alchemy.sql.elements
  # server_default setea el valor por defecto en la base de datos mientras que default setea el valor en el backend
  # osea cuando se va a agregar o actualizar el backend le agrega ese valor manualmente
  # cuando agregamos una nueva columna y la tabla ya existe al utilizar un valor por defecto lo que hara sera
  # colocar a todos los registros ese valor
