...
async function crearProductoRequest(body){
  const solicitud = await fetch('http://127.0.0.1:5000/producto', {method:'POST', body: JSON.stringify(body), headers:{
    // los headers sirven para enviar una encabeza de la peticion es decir gracias a estos el backend sabra que
    // informacion recibe, cuestiones de autenticacion, el origen de la peticion entre otros
