...
export const validarToken = (req, res, next)=>{
  ...
  // ahora en el header se authorization tenemos que validar que se envie en el formato Bearer xxxx.yyyy.zzzz
  const token = authorization.split(' ')[1] // ['Bearer', 'xxxx.yyyy.zzzz']
  if (!token) {
    return res.status(403).json({
  ...
  // verifica que la token sea correcta, que sea nuestra (por la firma), que siga vigente, cumpla con un formato valido
  try{
