...
tipoPlatoOpciones = [
  # En el caso de las opciones (choise) la primera opcion es la que se usara para el modelo y si estamos
  # utilizando formularios (django forms) se mostrara la segunda opcion, solo en ese caso, sin embargo
  # si utilizamos DRF o devolvemos la informacion en formato rest siempre se usara la primera
  ('entrada','ENTRADA'),
...
# vamosa utilizar el modelo auth_user proveniente del panel administrativo
# PermissionsMixin > sirve para indicarle a nuestro proyecto que ahora esta tabla sera la encargada de 
# manejar el sistema de permisos, es decir ahi se centraran todos los permisos que demos a nuestra aplicacion
...
class Cheff(AbstractBaseUser, PermissionsMixin):
  ...
  # EmailFields > a parte de guardar como un texto en la base de datos añade validacion para que el texto
  # ingresado sea un correo
  correo = models.EmailField(unique=True, null=False)
  # si vamos a configuar la password si o si tiene que llamarse password
  ...
  # vamos a modificar el comportamiento del atributo objects para que pueda aceptar una creacion de 
  # superusuario por consola
